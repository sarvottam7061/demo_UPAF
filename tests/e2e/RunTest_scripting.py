...
import argparse
import subprocess
from utilities.PyAutoRunner import get_pytest_run_command, get_pytest_run_command_e2e
...
my_parser.add_argument('-g',
                       '--bdd_url',
                       action='store',
                       help='url to override the bdd url mentioned in Test config')

my_parser.add_argument('-u',
                       '--web_url',
                       action='store',
                       help='url to override the web url mentioned in Test config')
...
my_parser.add_argument('-db',
                       '--db_uri',
                       action='store',
                       help='test uri to override the db uri mentioned in Test config')

my_parser.add_argument('-dp',
                       '--desktop_app_path',
                       action='store',
                       help='app path to desktop app')

my_parser.add_argument('-da',
                       '--desktop_app_title',
                       action='store',
                       help='App title for the desktop app')

my_parser.add_argument('-wu',
                       '--winapp_driver_cap',
                       action='store',
                       help='winappdriver application capability')

my_parser.add_argument('-wc',
                       '--winapp_driver_url',
                       action='store',
...
run_command, allure_result_path, allure_report_path, html_report_path, failure_analysis_path = get_pytest_run_command_e2e(args,
                                                                                                   run_mode_type="scripting",
                                                                                                   run_app_type="e2e",
                                                                                                   script_folder="file_e2e")
